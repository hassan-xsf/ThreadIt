-- ============================================================================
-- ThreadIt sample data
-- Reddit-style seed: communities (subforums), users, posts, nested comments,
-- votes and community memberships. All timestamps are backdated so the
-- platform looks lived-in.
--
-- Run:  psql "$DIRECT_URL" -f prisma/seed.sql
--   or: npm run db:seed
--
-- Idempotent: safe to re-run (uses ON CONFLICT DO NOTHING on stable ids).
-- Images use source.unsplash.com so they resolve without API keys.
-- ============================================================================
...
INSERT INTO "Community" ("id", "name", "description", "banner", "profile", "createdAt") VALUES
  ...
  ('com_technology', 'technology',
    ...
    '2023-11-10 12:30:00'),
  ('com_gaming', 'gaming',
    'A subforum for (almost) anything related to games â€“ video games, board games, card games, etc.',
...
-- ----------------------------------------------------- Memberships (m2m) ---
-- Prisma implicit relation table "_CommunityToUser": A = Community.id, B = User.id
...
-- ------------------------------------------------------------------ Votes ---
-- Post votes (unique per user+post+comment). commentId is NULL for post votes.
...
-- Comment votes. postId is NULL for comment votes.
INSERT INTO "Vote" ("id", "type", "userId", "postId", "commentId") VALUES
